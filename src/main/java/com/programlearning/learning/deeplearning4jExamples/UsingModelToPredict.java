package com.programlearning.learning.deeplearning4jExamples;
...
public class UsingModelToPredict {
    ...
    private static String modelPath = "src/main/resources/mnist/lenetmnist.zip";

    public static void main(String[] args) throws IOException {

        // 首先我们需要创建一个数据读取器,channel为1的时候，如果我们输入的一个彩色图，会自动的进行图像的灰度处理
        NativeImageLoader imageLoader = new NativeImageLoader(height, width, channel);
        // 指定测试的图片位置
        File imgFile = new ClassPathResource("/mnist/1.jpg").getFile();
         ...
         * 把灰度化之后的图片进行保存
         */
        ImageIO.write(imageFromINDArray(imgNdarray), "jpg", new File("src/main/resources/mnist/test.jpg"));
